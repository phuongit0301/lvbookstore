...
class XMPPHP_XMPP extends XMPPHP_XMLStream {
	...
	public function __construct($host, $port, $user, $password, $resource, $server = null, $printlog = false, $loglevel = null) {
		...
		//$this->addXPathHandler('{http://jabber.org/protocol/muc}presence', 'presence_handler');
	...
	public function message($to, $body, $type = 'chat', $subject = null, $payload = null) {
		...
		//$body	= htmlspecialchars($body);	//atan
	 ...
	 * @param int $seconds
	 */
	public function roomPresence($to = null, $seconds=0) 
	{
		$time = time();
		$to	 = htmlspecialchars($to);
		$this->log->log("roomPresence TO: $to",  XMPPHP_Log::LEVEL_DEBUG);

		$out = "<presence from=\"{$this->fulljid}\" id=\"$time\"";
		...
		$out .=">";
		$out .= "<x xmlns=\"http://jabber.org/protocol/muc\">";
		$out .= "<history seconds=\"$seconds\"/>";
		$out .= "</x>";
		$out .= "</presence>";

		$this->log->log("roomPresence out: $out",  XMPPHP_Log::LEVEL_DEBUG);
		$this->send($out);
	}
	...
	public function presence_handler($xml) {
		...
		if(strcmp($payload['type'], "error") == 0)
		{
			$this->log->log("Error in Room Presence!",  XMPPHP_Log::LEVEL_ERROR);
		}
